...
const RECS: Record<Season, {
  regions: RegionKey[];
  blurb: string;        // short line (kept for reuse)
  long: string;         // richer narration
  bestMonths: string;   // timing hint
}> = {
  spring: {
    regions: ['kansai', 'kanto', 'chubu'],
    blurb: 'Blossoms + mild temps: Kyoto/Nara, Tokyo+Nikkō, and Japan Alps; Tōhoku blooms late Apr–May.',
    long:
      "Cherry blossoms and soft light—perfect for slow temple walks, canal-side cafés, and mountain day hikes. Expect cool mornings, picnic-ready afternoons, and golden-hour photos that just hit different. Kyoto and Tokyo shine now; the Alps sprinkle in snow-capped backdrops.",
    bestMonths: 'Late Mar – mid Apr (Tōhoku: late Apr – early May)',
  },
  summer: {
    regions: ['hokkaido', 'okinawa', 'tohoku'],
    blurb: 'Cooler north + beach south: Hokkaidō festivals, Okinawa snorkeling, Tōhoku matsuri.',
    long:
      "Two perfect vibes: breezy Hokkaidō fields and festival nights up north, or tropical blues and reef snorkeling in Okinawa. Long daylight, fireworks, street food, and island time—pick cool air or warm seas (or be extra and do both).",
    bestMonths: 'Jul – Sep (Okinawa beaches peak Jun – Sep)',
  },
  autumn: {
    regions: ['kansai', 'tohoku', 'chugoku'],
    blurb: 'Maple season: Kyoto temples, Tōhoku color, and Chūgoku’s gorges/temples (Kankakei, Miyajima).',
    long:
      "Crimson maples, crisp mornings, and lantern-lit evenings. Kyoto lanes glow, Tōhoku valleys burn with color, and coastal trails in Chūgoku feel storybook. Comfort-food season, onsen steam, and photo ops basically everywhere.",
  ...
  winter: {
    ...
    long:
      "Snow-country magic up north—powder stashes, hot springs under falling snow, and cozy towns at blue hour. Or slide south to Kyūshū for milder days and steamy rotenburo. Illuminations, comfort ramen, and calm sights everywhere.",
...
export default function ItineraryPage() {
  ...
  const seasonKey = useMemo(() => toSeasonKey(selectedSeason || ''), [selectedSeason]);

  const handleProceed = () => {
    if (!region) return;
    const key = region.toLowerCase(); // e.g. "Kyushu" -> "kyushu"
    const params = new URLSearchParams();
    params.set('theme', includeThemePark ? '1' : '0'); // optional: let next page default filter
...
<style jsx>{`
  ...
  .season-pill:hover { transform: translateY(-1px); border-color: #fff; }
  .season-pill:active { transform: translateY(0); }

  .season-pill:focus-visible {
    box-shadow: 0 0 0 3px rgba(236,72,153,0.35); /* pink ring */
  }
  ...
  .season-pill:hover .bg { opacity: 0.85; }
  .season-pill.is-active .bg { opacity: 1; }
  ...
  .season-pill.is-active .glow { opacity: 0.35; }
  ...
  .season-pill:hover .emoji { transform: translateY(-2px) scale(1.08); }
  .season-pill.is-active .emoji { animation: pop 260ms ease; }

  .season-pill .underline {
    position: absolute; left: 50%; bottom: 3px; height: 3px;
    transform: translateX(-50%);
    border-radius: 9999px;
    width: 0; opacity: 0;
    transition: width 220ms ease, opacity 200ms ease;
  }
  .season-pill.is-active .underline { width: 2.5rem; opacity: 1; }

  /* Gradients per season */
  .season-spring .bg,
  .season-spring .glow,
  .season-spring .underline { background-image: linear-gradient(90deg,#ec4899,#f43f5e,#ec4899); }

  .season-summer .bg,
  .season-summer .glow,
  .season-summer .underline { background-image: linear-gradient(90deg,#fde047,#fb923c,#fde047); }

  .season-autumn .bg,
  .season-autumn .glow,
  .season-autumn .underline { background-image: linear-gradient(90deg,#f59e0b,#ef4444,#f59e0b); }

  .season-winter .bg,
  .season-winter .glow,
  .season-winter .underline { background-image: linear-gradient(90deg,#22d3ee,#6366f1,#22d3ee); }
...
`}</style>
        ...
        {/* Season banner — inviting, big, dynamic */}
{seasonKey && (
  <div className="relative overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900/60 p-6 sm:p-8 mb-8">
    ...
    <div className={`absolute inset-0 bg-gradient-to-r blur-2xl opacity-20 animate-pulse
      ${seasonKey === 'spring' ? 'from-pink-500 to-rose-500'
        : seasonKey === 'summer' ? 'from-yellow-400 to-orange-500'
        : seasonKey === 'autumn' ? 'from-amber-600 to-rose-700'
        : 'from-cyan-400 to-indigo-600'}`} aria-hidden
    />
    <div className="relative max-w-3xl mx-auto text-center">
      ...
      <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
        Plan a <span className="bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-rose-500">
          {seasonKey === 'spring' ? 'Spring'
            : seasonKey === 'summer' ? 'Summer'
            : seasonKey === 'autumn' ? 'Autumn'
            : 'Winter'}
        </span> escape you’ll love
      </h2>
        ...
        <div className="mt-10">
          <button
            ...
            className={`px-6 py-3 rounded-xl text-lg font-semibold shadow transition
              ${region ? 'bg-white text-neutral-900 hover:bg-neutral-200' : 'bg-neutral-700 text-neutral-400 cursor-not-allowed'}`}
          >
